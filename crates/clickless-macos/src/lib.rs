...
impl<O: OutputBackend> MacosHook<O> {
    ...
    pub fn process_key(&mut self, code: u16, is_down: bool, now_ms: u64) -> Option<Action> {
        let key = key_code::cg_to_logical(code)?;
...
mod tests {
    ...
    fn enter_mouse(hook: &mut MacosHook<MockOut>) {
        hook.process_key(0x39, true, 0); // CapsLock press
        hook.process_key(0x39, true, 200); // poll past threshold
    }

    #[test]
    fn t01_unmapped_code_yields_none() {
        let mut hook = MacosHook::new(MockOut::new());
        assert_eq!(hook.process_key(0x24, true, 0), None); // kVK_Return
    }

    #[test]
    fn t02_capslock_hold_enters_mouse_layer() {
        let mut hook = MacosHook::new(MockOut::new());
        hook.process_key(0x39, true, 0);
        hook.process_key(0x39, true, 200);
    ...
    fn t03_j_in_mouse_yields_move_right() {
        ...
        assert_eq!(hook.process_key(0x26, true, 300), Some(Action::MoveRight));
    }
    ...
    fn t04_h_in_mouse_yields_move_left() {
        let mut hook = MacosHook::new(MockOut::new());
        enter_mouse(&mut hook);
        assert_eq!(hook.process_key(0x04, true, 300), Some(Action::MoveLeft));
    }
    ...
    fn t05_k_in_mouse_yields_move_up() {
        let mut hook = MacosHook::new(MockOut::new());
        enter_mouse(&mut hook);
        assert_eq!(hook.process_key(0x28, true, 300), Some(Action::MoveUp));
    }
    ...
    fn t06_l_in_mouse_yields_move_down() {
        let mut hook = MacosHook::new(MockOut::new());
        enter_mouse(&mut hook);
        assert_eq!(hook.process_key(0x25, true, 300), Some(Action::MoveDown));
    }
    ...
    fn t07_f_in_mouse_yields_click_left() {
        let mut hook = MacosHook::new(MockOut::new());
        enter_mouse(&mut hook);
        assert_eq!(hook.process_key(0x03, true, 300), Some(Action::ClickLeft));
    ...
    fn t08_d_in_mouse_yields_click_right() {
        let mut hook = MacosHook::new(MockOut::new());
        enter_mouse(&mut hook);
        assert_eq!(hook.process_key(0x02, true, 300), Some(Action::ClickRight));
    ...
    fn t09_w_in_mouse_yields_scroll_up() {
        let mut hook = MacosHook::new(MockOut::new());
        enter_mouse(&mut hook);
        assert_eq!(hook.process_key(0x0D, true, 300), Some(Action::ScrollUp));
    ...
    fn t10_s_in_mouse_yields_scroll_down() {
        let mut hook = MacosHook::new(MockOut::new());
        enter_mouse(&mut hook);
        assert_eq!(hook.process_key(0x01, true, 300), Some(Action::ScrollDown));
        assert_eq!(hook.out().scrolls, vec![(0, -1)]);
    }
    ...
    fn t11_tick_moves_cursor() {
        let mut hook = MacosHook::new(MockOut::new());
        enter_mouse(&mut hook);
        hook.process_key(0x26, true, 300); // J
        hook.tick(100);
    ...
    fn t12_esc_exits_mouse_layer() {
        let mut hook = MacosHook::new(MockOut::new());
        enter_mouse(&mut hook);
        hook.process_key(0x35, true, 300); // Esc
    ...
    fn t13_capslock_release_exits_mouse() {
        let mut hook = MacosHook::new(MockOut::new());
        enter_mouse(&mut hook);
        hook.process_key(0x39, false, 400); // CapsLock release
        assert_eq!(hook.sm().layer(), clickless_core::Layer::Initial);
    }
    ...
    fn t14_u_in_mouse_yields_speed_down() {
        let mut hook = MacosHook::new(MockOut::new());
        enter_mouse(&mut hook);
        assert_eq!(hook.process_key(0x20, true, 300), Some(Action::SpeedDown));
    }
    ...
    fn t15_o_in_mouse_yields_speed_up() {
        let mut hook = MacosHook::new(MockOut::new());
        enter_mouse(&mut hook);
        assert_eq!(hook.process_key(0x1F, true, 300), Some(Action::SpeedUp));
    }
}
